...
class _LanguageEditorSheetState
  ...
  late final Map<String, LanguageConfig>
      _languageByCode;
...
String _languageKeyOf(
  ...
  Map<String, dynamic>?
    ...
    if (language != null) {
      ...
      final savedScriptCode =
        _scriptCodeOf(result);

      if (savedScriptCode == null &&
          language.scriptCodes.isNotEmpty) {
        result['scriptCode'] =
  ...
  Future<void> _handleAddLanguage(
  LanguageConfig language,
) async {
  final scripts =
      language.scriptCodes
          .map(
            ScriptConfig.findByCode,
          )
...
void _syncLanguageScripts(
  ...
  for (final item
      in _selectedLanguages) {
    if (_languageCodeOf(item) !=
        languageCode) {
      continue;
    }
  ...
  setState(() {
    ...
    for (final scriptCode
        in selectedScriptCodes) {
      final key = _languageKey(
        language.code,
        scriptCode,
      );

      _selectedLanguageKeys.add(
        key,
      );
...
String _scriptOptionName(
  LanguageConfig language,
  ScriptConfig script,
) {
  return language.scriptNameOf(
    script.code,
    _uiLanguageCode,
  );
}
  ...
  void _removeLanguageAt(
    ...
    final key =
    _languageKeyOf(language);

    setState(() {
      _selectedLanguages.removeAt(
        index,
      );

      _selectedLanguageKeys.remove(
        key,
      );

      if (_expandedLanguageKey ==
    key) {
  _expandedLanguageKey =
      null;
}
    });
  }

  void _removeLanguageByCode(
    String code,
  ) {
  ...
  void _removeLanguageByKey(
  String key,
) {
  final index =
      _selectedLanguages.indexWhere(
    (language) =>
        _languageKeyOf(language) ==
        key,
  );
            ...
            children: [
              ...
              Expanded(
                child:
                    TabBarView(
                  children: [
                    _buildSelectedLanguagesTab(
                      context,
                    ),
                    _buildLanguageLibraryTab(
                      context,
                    ),
                  ],
                ),
  ...
  final scriptCode =
      _scriptCodeOf(languageData);

  if (scriptCode == null) {
    return languageConfig.nameOf(
      _uiLanguageCode,
    );
  }

  final scriptName =
      languageConfig.scriptNameOf(
    scriptCode,
    _uiLanguageCode,
  );
