...
if (status == 'deleted') {
  ...
  switch (action) {
    ...
    case 'delete':
      await _showDeleteMessageOptions(
        messageId: messageId,
        isMe: isMe,
      );
      break;
...
Future<void> _loadLiveDraftSetting() async {
  ...
  try {
    ...
    final enabled =
        await chatProvider.getLiveDraftEnabled(
      chatId: widget.chatId,
      userId: userId,
    );
...
Future<void> _initializeChat() async {
  ...
  // 也不能影响实时预览。
  final draftsStream =
      _liveDraftService.watchChatDrafts(
    chatId: widget.chatId,
    currentUserId: currentUserId,
  );
...
Future<void> _loadOtherUserData() async {
  ...
  final userIds =
      await chatProvider.getChatParticipants(
    widget.chatId,
  );
...
Future<void> _loadParticipantData() async {
  ...
  for (final userId in allUserIds) {
    try {
      ...
      loadedData[userId] = UserModel.fromJson({
        ...data,
        'uid': doc.id,
      });
...
AppBar _buildAppBar() {
  ...
  final chatProvider =
      context.watch<chatProv.ChatProvider>();

  return StreamBuilder<QuerySnapshot>(
    stream: chatProvider.watchMessages(
      widget.chatId,
    ),
    builder: (context, snapshot) {
